<template>
  <div ref="container" id="score-player" style="background-color: #fff;padding: 0;margin: 0">
    <canvas id="canvas" ref="canvas" :width="playerWidth" :height="playerHeight" style="background-color: #6f7180;display: block"></canvas>
    ...
    <!--<div
      ...
      <div :class="{'key-btn-v': direction===0, 'key-btn-h': direction===1}" title="横竖屏" style="float: right;margin-right: 20px" @click="handleDirectionChange"/>
      <div :class="{'page-btn-1': pagePer===1, 'page-btn-2': pagePer===2,'page-btn-3': pagePer===3}" title="切换横屏页数" style="float: right;margin-right: 20px" @click="handlePagePerChange"/>
      <div
        :class="{'cursor-btn': player.showCursor,'cursor-prevent': !player.showCursor}"
        title="光标"
...
export default {
  ...
  data() {
    return {
      ...
      assets: {
        leftImgPath: "http://yulindb.oss-cn-beijing.aliyuncs.com/score_player/left.png?x-oss-process=style/assets_img",
        leftPressImgPath: "http://yulindb.oss-cn-beijing.aliyuncs.com/score_player/left_p.png?x-oss-process=style/assets_img",
        rightImgPath: "http://yulindb.oss-cn-beijing.aliyuncs.com/score_player/right.png?x-oss-process=style/assets_img",
        rightPressImgPath: "http://yulindb.oss-cn-beijing.aliyuncs.com/score_player/right_p.png?x-oss-process=style/assets_img",
        loadImgPath: "http://yulindb.oss-cn-beijing.aliyuncs.com/score_player/load.png?x-oss-process=style/assets_img",
      }
    }
  },
  watch: {
  ...
  mounted() {
    ...
    this.initPlayer(function(event) {
      if (event.type === 'onImgLoadProgress') { // 乐谱图片加载进度
        ...
        _this.player.timeText = ('正在加载音频，请稍后，当前进度：' + progress + '%');
      } else if (event.type === 'onAudioLoadComplete') { // 音频加载完成
        soundInstance = event.value;
        _this.player.timeText = ('00:00 / ' + _this.formatTime(soundInstance.duration / 1000));
        ...
        if (soundInstance) {
          _this.player.timeText = (_this.formatTime(soundInstance.position / 1000) + ' / ' + _this.formatTime(soundInstance.duration / 1000));
        }
